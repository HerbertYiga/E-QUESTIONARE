...
public class LoginController {
	...
	public ModelAndView checkUser(HttpServletRequest request, HttpSession session, UserBean user, Model model)
		...
		Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/questionnaire", "root", "onlylord");
		...
		if (((user.getPassword().equals("admin123")) && ((user.getUserName().equals("Admin"))))) {
			...
			model.addAttribute("error", "please insert a correct user name and password");
